...
export async function POST(request: Request) {
  try {
    ...
    if (text === '/agendamentos' || text === '/start') {
      ...
      if (todayAppointments.length > 0) {
        responseMessage = `✨ <b>Agenda VIP - Hoje</b> ✨\n\n` +
    ...
    return NextResponse.json({ ok: true }); // Sempre retorna ok para o Telegram não repetir a entrega
  }
}